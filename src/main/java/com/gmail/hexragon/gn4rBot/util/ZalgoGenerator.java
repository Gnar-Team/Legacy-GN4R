...
public class ZalgoGenerator
{
	...
	private static char randomChar(char[] array)
	{
	...
	{
		for (char aZalgo_up : zalgo_up) if (c == aZalgo_up)
		{
			return true;
		}
		for (char aZalgo_down : zalgo_down) if (c == aZalgo_down)
		{
			return true;
		}
		for (char aZalgo_mid : zalgo_mid) if (c == aZalgo_mid)
		{
			return true;
		}
		return false;
	...
	public static String process(String iText, boolean up, boolean down, boolean mid)
	{
		String zalgoTxt = "";
...
//			num_down = random.nextInt(8);
			...
			if (down)
			{
				...
				{
					zalgoTxt += randomChar(zalgo_down);
				}
			}
		}
